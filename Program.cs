...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        bool
            _shouldFire = false,
            _shouldKill = false,
            _shouldCruise = true, // Used if the missile should gain alt to hit the enemy (used by this version for the AIM-100 )
            _hasPassed = false,
            _killAllowed = false,
            _shouldDive = false,
            _shouldStealth = false,
    ...
    "We've been trying to reach you about your car's extended warranty",
        ...
        enum LauncherType : int { Ground = 0, Air = 1 };
        ...
        ConfigDouble _guidanceDelay = new ConfigDouble(IniSectionDelays, "Guidance delay (s)", 0.2);
        ConfigDouble _detachDuration = new ConfigDouble(IniSectionDelays, "Stage 2: Detach duration (s)", 0.1);
        ConfigDouble _mainIgnitionDelay = new ConfigDouble(IniSectionDelays, "Stage 3: Main ignition delay (s)", 0.1);
            ...
            _guidanceAlgorithms = new Dictionary<GuidanceAlgoType, MissileGuidanceBase>()
{
    ...
    { GuidanceAlgoType.ZeroEffortMiss, new ZeroEffortMissGuidance(UpdatesPerSecond, _navConstant) },
        ...
        double CalculateCruiseHeight(Vector3D targetPosition, Vector3D shooterPosition, Vector3D currentPosition, double cruiseHeight)
        {
            // Vector3D direction = Vector3D.Normalize(targetPosition - shooterPosition);
            // Convert 3D positions to 2D relative X values
            double distance = Vector3D.Distance(targetPosition, shooterPosition); //Vector3.Dot(shooterPosition - currentPosition, forward);
            double missileDistTarget = Vector3D.Distance(currentPosition, targetPosition);
            double missileDistLaunch = Vector3D.Distance(currentPosition, shooterPosition);
            double T = Vector3D.Distance(targetPosition, currentPosition);//Vector3.Dot(targetPosition - currentPosition, forward);

            Vector2D V1 = new Vector2D(0, 0);
            Vector2D V2 = new Vector2D(distance, 0);
            Vector2D V3 = new Vector2D(distance / 2, cruiseHeight);

            double r = -(5000 * distance) / cruiseHeight;
            Func<double, double> f = (x) =>
            {
                double term1 = -r / Math.Pow(V3.X - V2.X, 2);
                double term2 = Math.Pow(x - V3.X, 2);
            ...
            // for testing. I believe the parbolic descent equation is incorrect.
            _cruiseHeight = 3000;
            double m = Math.Sqrt(Math.Pow(V1.Y - _cruiseHeight, 2)) / Math.Sqrt(Math.Pow(V1.X - V3.X, 2));
            result = m * T - V1.Y;
            if (result < 300)
                result = 0;
            return result;
        }
        // Vector3D origin = _missileReference.TryGetPlanetPosition(out Vector3D planetOrigin);
        // Vector3D upVector = Vector3.Normalize(currentPosition - origin)
        // Vector3D target = (upVector * cruiseHeight) + targetPos?
        ...
        public class TargetingComputer
        {
            ...
            public TargetingComputer(IMyShipController missileController, List<IMyThrust> mainThrusters)
            {
                ...
                /*
                 Other init, going to be used for the Automated-Air Defense System
            ...
            public void _update(Vector3D targetPosition, Vector3D targetVelocity, double timeSinceLastIngest, double certaincy)
            {
                ...
                _cruiseHeight = _Trajectory._update(targetPosition, _missilePosition, _missileController);
            }
            public Vector3D _getAdjustedFlight(Vector3D acclCmd)
            {
            ...
            {
                /*
                 Complicated set of equations to check
            ...
            private LauncherType _determineLauncherType()
            {
                ...
                if (distFromSurface < 50)
                    return LauncherType.Ground; // On Ground
                return LauncherType.Air; // In Air over 50 meters.
            }
            /// <summary>
            ///  Calculates the newtons needed from the force of the thruster to maintain sub-hypersonic flight.
            /// </summary>
            /// <param name="missileVelocity"> </param>
            /// <returns></returns>
            private double _hypersonicCruise()
            {
                double missileThrust = _calculateMissileThrust();
                Vector3D planetPosition;
                _missileController.TryGetPlanetPosition(out planetPosition);

                if (planetPosition != new Vector3D())
                {
                    double height;
                    double alt; // not yet used, but will be used for calculating cruiseSpeed.
                    _missileController.TryGetPlanetElevation(MyPlanetElevation.Surface, out height);
                    _missileController.TryGetPlanetElevation(MyPlanetElevation.Sealevel, out alt);
                    double actualSpeed = Math.Sqrt(Math.Pow(_missileVelocity.X, 2) + Math.Pow(_missileVelocity.Y, 2) + Math.Pow(_missileVelocity.Z, 2)); // Relative to 0,0,0 how fast are we going.
                    double cruiseSpeed = 400; // speed to maintain m/s
                    double cruiseThrust = ((0.5 * _missileMass * Math.Pow(cruiseSpeed, 2)) - (0.5 * _missileMass * Math.Pow(_missileController.GetShipSpeed(), 2))) / (_timeSinceLastIngest);
                    //  if (cruiseThrust < 0)
                    //      return 0; // cant have negative thrust.
                    // Temp
                    if (height > 5000)
                        return cruiseThrust;
                    else
                        return missileThrust / _missileMass;
                  //  return missileThrust / _missileMass;//cruiseThrust;
                }
                else
                    return missileThrust / _missileMass;

            }
            public double _calculateMissileThrust()
            {
                double thrust = 0;
                foreach (var block in _thrusters)
                {
                    if (block.Closed)
                        continue;
            ...
            private class Trajectory
            {
                ...
                private class Path
                {
                    ...
                    private void slicePoints(Vector3D upVector)
                    {
                        ...
                        V3 = new Vector2D(distance / 2, b / 2);
                    }
                    ...
                    public double PathSolve(Vector3D targetPosition, Vector3D currentPosition, Vector3D planetOrigin, double planetRadius)
                    {
                        ...
                        // Calculate the target position above sea level
                        //Vector3D targetPosition = currentPosition + Vector3D.Normalize(currentPosition - planetOrigin) * (heightFromSeaLevel + __cruiseHeight);
                        // Calculate the direction vector from the planet origin to the target

                        // Now, targetPositionAboveSurface represents the position where the missile should cruise:
                        // - It will follow the target's direction.
                        // - It will maintain a fixed height above the planet's surface (cruiseHeight).
            ...
            out Vector3D adjustedTargetPos)
        {
            ...
            _TargetingComputer._update(adjustedTargetPos, _targetVel, _timeSinceLastIngest, _timeSinceLastLock); // Multiply the units by the force generated to maintain hypersonic flight.  
            ...
            if (_shouldCruise && !_topDownAttack) //Factor in limits and piecewise to pick different trajectories depending on flight progress.
            {
                ...
                adjustedTargetPos += Vector3D.Normalize(gravityVec) * -_TargetingComputer._getCruiseHeight();
            }
            ...
            return _TargetingComputer._getAdjustedFlight(accelCmd);
        }
        ...
        */
        ...
        class RaycastHoming
        {
            ...
            public Vector3 PredictPosition(Vector3 initialVelocity, Vector3 acceleration, float timeSinceDataGathered, Vector3 lastPosition)
            {
                float timeSquared = timeSinceDataGathered * timeSinceDataGathered;
                Vector3 predictedPosition = lastPosition + initialVelocity * timeSinceDataGathered + 0.5f * acceleration * timeSquared;

                return predictedPosition;
            }

            public Vector3 CalculateProportionalNavigation(Vector3 missilePosition, Vector3 targetPosition, Vector3 targetVelocity)
            {
                Vector3 relativePosition = targetPosition - missilePosition;
                Vector3 lineOfSightRate = Vector3.Cross(relativePosition, targetVelocity) / Vector3.Dot(relativePosition, relativePosition);
                Vector3 guidanceCommand = lineOfSightRate;

                return guidanceCommand;
            }

            public Vector3 CalculateLeadPursuit(Vector3 missilePosition, Vector3 targetPosition, Vector3 targetVelocity)
            {
                Vector3 relativePosition = targetPosition - missilePosition;
                float timeToIntercept = relativePosition.Length() / targetVelocity.Length();
                Vector3 leadInterceptPoint = targetPosition + targetVelocity * timeToIntercept;

                return leadInterceptPoint;
            }

            public bool AttemptLIDARPing(Vector3 position)
            {
                // Simulate LIDAR ping logic
                return true;
            }
            public Vector3D calculateTargetPosition()
            {
                return TargetCenter + TargetVelocity * TimeSinceLastLock;
            }
            public void Update(double timeStep, List<IMyCameraBlock> cameraList, List<IMyShipController> shipControllers, IMyTerminalBlock referenceBlock = null)
            {
                ...
                switch (_currentAimMode)
                {
                    ...
                    case AimMode.OffsetRelative:
                        ...
                        break;
                    default:
                        scanPosition = this.calculateTargetPosition();
                        break;
                }
